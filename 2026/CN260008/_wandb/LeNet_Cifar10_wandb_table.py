...
def parse_args():
    ...
    parser.add_argument('--watch_model', action='store_true', default=True, help='') # 修改为默认 True 
    
    return parser.parse_args()

@torch.no_grad()
def evaluate(val_loader, model, epoch, use_wandb=False, table=None, 
             logger_epoch_interval=5, num_batchs_to_log=10, num_sample_to_log_per_batch=16):
    """
    Args:
    ...
    for input, label in val_loader:
        ...
        if use_wandb_table and (batch_id < num_batchs_to_log):
            ...
            n1 = num_sample_to_log_per_batch
            log_images, log_labels, log_preds = input[:n1].permute(0, 2, 3, 1).detach().cpu().numpy(), list(classnames[label[:n1].detach().cpu().numpy()]), list(classnames[pred_label[:n1].detach().cpu().numpy()])
            sample_id = 0
            for log_image, log_label, log_pred in zip(log_images, log_labels, log_preds):
                id = f"{epoch}_{batch_id}_{sample_id}"
                table.add_data(id, wandb.Image(log_image), log_label, log_pred) # 图片形状需为 (H, W, C)，故之前 permute 将 (B, C, H, W) 转为 (B, H, W, C)
                sample_id += 1
            batch_id += 1
...
def main():
    ...
    for epoch in range(args.epochs):
        ...
        test_acc = evaluate(val_loader, model, epoch, use_wandb=args.use_wandb, table=table)
